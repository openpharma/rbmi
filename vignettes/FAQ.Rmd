...
can be customised via the `group_contrasts` argument of `set_vars()`. For example, with
three arms `"Placebo"`, `"Low"` and `"High"` (in that factor-level order) you can request
a bespoke set of pairwise contrasts:
...
vars <- set_vars(
    ...
    group_contrasts = list(
        c("Low", "Placebo"),   # -> trt          (alt  vs ref)
        c("High", "Placebo"),  # -> trt_alt2     (alt2 vs ref)
        c("High", "Low")       # -> trt_alt2_alt (alt2 vs alt)
    )
)
