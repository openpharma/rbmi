...
#'
#' @param scale The scale of results returned by `as.data.frame.pool()`. Must
#' be `"auto"` (the default), `"original"`, or `"transformed"`. `"auto"` uses
#' transformed results when available and original results otherwise.
#'
#' @param ... not used.
#'
#' @details
...
print.pool <- function(x, ...) {
    ...
    results_header <- if (is.null(x$transformed_pars)) {
        "Results:"
    } else {
