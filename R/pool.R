...
#' @param v_com Positive number representing the degrees of freedom in the complete-data analysis.
#' @param method Degrees-of-freedom approximation to use: `"barnard-rubin"` applies
#' the Barnard-Rubin (1999) small-sample adjustment and `"rubin"` applies
...
rubin_rules <- function(
    ...
    assert_that(
        length(method) == 1,
        method %in% c("barnard-rubin", "rubin"),
        msg = "`method` must be either 'barnard-rubin' or 'rubin'"
    )
    M <- length(ests)
    est_point <- mean(ests)
